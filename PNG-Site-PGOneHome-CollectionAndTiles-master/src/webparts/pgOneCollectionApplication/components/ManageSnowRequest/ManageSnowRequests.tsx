...
import styles from './ManageSnowRequests.module.scss';
import { SPPermission } from '@microsoft/sp-page-context';
import Pagination from 'office-ui-fabric-react-pagination';
import { sp } from '@pnp/sp';
import { stringIsNullOrEmpty } from '@pnp/common';
interface IManageSnowRequestsList {
  Title: string;
  ID?: string;
}
...
interface IManageSnowRequestsState {
  status: string;
  resourceListItems: any[];
  myTilesItems: IManageSnowRequestsList[];
  currentItem: IManageSnowRequestsList[];
  columns: IColumn[];
  ...
  hideCollectionDialog:boolean;

}

interface IManageSnowRequestsProps {
...
export class ManageSnowRequests extends React.Component<IManageSnowRequestsProps, IManageSnowRequestsState> {

  private pnpHelper: PnPHelper;
  private _allItems: IManageSnowRequestsList[];
  ...
  private itemsForPagination: IManageSnowRequestsList[];
  ...
  private errModule: string = "ManageSnowRequests.tsx";

  constructor(props: IManageSnowRequestsProps, state: IManageSnowRequestsState) {
    super(props);
    ...
    const colummsForCollection: IColumn[] = [
      {
        key: 'ID', name: 'Sr. No', fieldName: 'ID',
        minWidth: 20, maxWidth: 60, isResizable: true, isCollapsible: true,
        //onColumnClick: this._onColumnClickSort.bind(this),
      ...
      {
        key: 'CollectionOwnerEmail', name: 'CollectionOwnerEmail',
        sortAscendingAriaLabel: 'Sorted A to Z',
        sortDescendingAriaLabel: 'Sorted Z to A',
        onColumnClick: this._onColumnClickSort.bind(this),
        isRowHeader: true,
        data: 'string',
        isPadded: true,
        fieldName: 'CollectionOwnerEmail', minWidth: 200, isResizable: true, isCollapsible: false,
      ...
      
      {
        ...
        onRender: (item) => (
          <div>
            ...
            <Link aria-label="Edit" title="Edit" onClick={() => { this.convertCollection(item); }} ><Icon iconName="Edit" /></Link>
    ...
    this.state = {
      ...
      collectionColumns: colummsForCollection,
      currentItem: [],
  ...
  public render(): React.ReactElement<IManageSnowRequestsProps> {
    return (
      <React.Fragment>
        <div>
          <div className={styles.ManageSnowRequests}>
            <div className={`ms-Grid`}>
              <div className="ms-Grid-row ">
                <div className="ms-Grid-col ms-sm10 ms-md12 ms-lg12 ms-xl12">
                  <h2 className="ms-font-m ms-fontWeight-semibold">{this.state.resourceListItems['ManageSnowRequests_label']}</h2>
                </div>
              </div>
              <hr role="presentation"></hr> 
              <div className={`ms-Grid-row ${styles.custRow}`}>
                <div className="ms-Grid-col ms-sm12 ms-md4 ms-lg4 ms-xl4">
                  <Dropdown
                    // placeholder="Select an option"
                    label=""
                    options={[{ key: 'Manage Tiles', text: 'Manage Tiles' }, { key: 'Make Collection Public to Private', text: 'Make Collection Public to Private' }]}
                    //styles={}
                ...
                <div className="ms-Grid-col ms-sm12 ms-md4 ms-lg3 ms-xl3">
                  {this.state.requestType == "Manage Tiles" ?
                    <Link>
                      <Icon iconName="AddTo" onClick={this.addNewTile.bind(this)} />
                    </Link>
                    : ""
                  }
                </div>
                ...
                <div className={`ms-Grid-row ${styles.gridRowDetailsList}`}>
                  <div className="ms-Grid-col ms-sm12 ms-md12 ms-lg12 ms-xl12" >
                    <DetailsList items={this.state.myTilesItems}
                      ...
                      columns={this.state.requestType == "Manage Tiles" ? this.state.columns : this.state.collectionColumns}
          ...
          <Panel
            isLightDismiss
            className={styles.ManageSnowRequestsPanel}
          ...
          {/* Add/Edit Tiles Panel */}
          <Panel
            isLightDismiss
            className={styles.ManageSnowRequestsEditPanel}
            headerText={this.state.resourceListItems["add_tile_manage_my_tiles_label"]}
         ...
         {/* To Convert Public Collection to Private */}
          <Dialog
            hidden={this.state.hideCollectionDialog}
            onDismiss={() => this.setState({ hideCollectionDialog: true })}
            dialogContentProps={{
              type: DialogType.normal,
              showCloseButton: false
            }}
          ...
          >
            <div className={styles.contentClass}>
                <div>
                  <Icon iconName="ChangeEntitlements"></Icon>
                  <h2>{this.state.resourceListItems["convertCollection_Dialog_Title"]} </h2>
                  <p>{this.state.resourceListItems["convertPublicToPrivate_Dialog_Text"]}</p>
                </div>
            </div>
            <DialogFooter className={styles["ms-Dialog-actionsButton"]}>
              <DefaultButton text={this.state.resourceListItems["add_tile_cancel_text"]} onClick={() => this.setState({ hideCollectionDialog: true })} />

             
                <PrimaryButton text={this.state.resourceListItems["covertCollection_Dialog_Yes_Button_Text"]} onClick={this.yesConvert.bind(this)} disabled={this.state.dissableRemoveButton} />
  ...
  private _onFilter = (searchValue: string): void => {
    let filteredApplicationTileList: IManageSnowRequestsList[] = [];
  ...
  private showEditPanel = (item: any): void => {
    ...
    this._refreshValidationData(item['Id']).then(()=>{
      this.setState({
        isRecordIsAlreadyInApproval: false,
  ...
  private yesDeleteItem = (): void => {
    ...
    try {
      sp.web.lists.getByTitle(this.ApplicationMasterListName)
        .items.getById(this.state.TileId).update({
          IsActive: false
        }).then(r => {
          this.props.callBackForRequestSection("Tile deleted sucessfully.");
          this.getMyAllTiles();
        });
    }
    catch (error) {
    ...
    try {
      ...
      // checking email and url type
        ...
        if (this.state.TileId == 0) {
          //add new tile directly in to the application master
          try {
            sp.web.lists.getByTitle(this.ApplicationMasterListName)
              .items.add({
                Title: this.state.TileTitle,
                OwnerEmail: this.state.TileOwner,
                Link: this.state.TileURLLink,
                Description: this.state.TileDescription,
                SearchKeywords: this.state.TileKewords,
                //ExistingItemID: parseInt(this.state.['TileId']),
                ColorCodeId: this.state.TileColorCodeId,
                //RequestedDate: new Date(),
                // RequestedAction: this.state.['RequestedAction'],
                AvailableExternal: this.state.TileAvailableExternal
              }).then(r => {
                this.setState({
                  showEditPanel: false,
                });
                this.getMyAllTiles();
                this.props.callBackForRequestSection("Tile added sucessfully.");
              });
          } catch (error) {
            //console.log(error);
          }
          ...
          //update the tile directly in to the application master
          try {
            sp.web.lists.getByTitle(this.ApplicationMasterListName)
              .items.getById(this.state.TileId).update({
                Title: this.state.TileTitle,
                ...
                this.getMyAllTiles();
                this.props.callBackForRequestSection("Tile updated sucessfully.");
              });
          } catch (error) {
            //console.log(error);
          }
  ...
  private addNewTile = (): void => {
    this._refreshValidationData(0).then(()=>{
      this.showError = false;
      this.setState({
        isRecordIsAlreadyInApproval: false,
        status: "",
  ...
  private async getPublicCollection(): Promise<void> {
    try {
      Promise.all([
      ...
      this.pnpHelper.errorLogging.logError(this.errTitle, this.errModule, "", error, "getPublicCollection");
    }

  }

  private convertCollection(item: any) {
      this.setState({
  ...
  private yesConvert = (): void => {
    this.setState({
      hideCollectionDialog: true,
    });
    try {
      sp.web.lists.getByTitle("CollectionMaster")
        .items.getById(this.state.CollectionId).update({
          IsActive: false
        }).then(r => {
          this.props.callBackForRequestSection("Collection updated sucessfully.");
          this.getPublicCollection();
        });
    }
    catch (error) {
      //console.log(e);
      this.pnpHelper.errorLogging.logError(this.errTitle, this.errModule, "", error, "yesConvert");
    }
